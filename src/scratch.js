...
jsPsych.plugins['canvas-sliders-response-new'] = (function() {

    let plugin = {};

    plugin.info = {
        name: 'canvas-sliders-response-new',
        ...
        parameters: {
            ...
            require_change_warning: {
                ...
                'To continue please report which box has more dots using the scale.'
                +'</span>'],
                array: true,
            ...
            drawSlidersWithStimulus: {
                type: jsPsych.plugins.parameterType.BOOL,
                pretty_name: 'Draw sliders with stimulus',
                array: false,
                default: true,
                description: 'Whether to draw the sliders with the stimulus onset (true) or offset (false).'
            },
            button_label: {
                type: jsPsych.plugins.parameterType.STRING,
    ...
    plugin.trial = function(display_element, trial) {
        ...
        if(trial.drawSlidersWithStimulus)
            html += drawSliders();
        ...
        let stimOffFun;
        if(trial.hideFunction === null) {
            stimOffFun = function(canvasContainer) {
                // Draw sliders late if necessary
                if(!trial.drawSlidersWithStimulus)
                    display_element.innerHTML += drawSliders();

                canvasContainer.style.visibility = 'hidden';
            }
        } else {
            stimOffFun = (canvasContainer) => {
                if(!trial.drawSlidersWithStimulus)
                    display_element.innerHTML += drawSliders();

                trial.hideFunction(canvasContainer);
            }
        }
